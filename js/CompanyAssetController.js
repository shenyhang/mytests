app.controller('CompanyAssetController',function($scope,httpService,$state,$rootScope){
		...
		.then(function success(resp){  
			...
			if(httpResponse(resp).success==1){
				if(httpResponse(resp).message.result.length!=0){
					...
					$scope.resetParam.address = $scope.result.addressobj[0].address;
					...
					$scope.result.addressobj[0].choosed = true;
	...
	//查询
	$scope.history = function(symbol,item,index){
		for(var i = 0 ;i<$scope.result.addressobj.length;i++){
			if(i==index){
				$scope.result.addressobj[index].choosed=true;
			}else{
				$scope.result.addressobj[i].choosed=false;
			}
		}
		$scope.resetParam.current = symbol;
		$scope.resetParam.condition = item.address;//$scope.resetParam.address;
		getBlockService($scope.resetParam);
	};
