app.controller('HotWalletController',function($scope,httpService,$state,$rootScope){
	...
	function getAddressService(reqParam){
		var header = getLocalToken();
		httpService.http('private','getHotWalleteInfo','get',reqParam,{},header)
		.then(function success(resp){  
			if(httpResponse(resp).success==1){
				$scope.result.addressflag=true;
				$scope.result.addressobj=httpResponse(resp).message.result;
				$scope.resetParam.address = $scope.result.addressobj.Hot_Address;
	...
	function getBlockService(reqParam){
		var header = getLocalToken();
		httpService.http('private','getBlockByConditionAndAsset','get',reqParam,{},header)
		.then(function success(resp){ 
			if(httpResponse(resp).success==1){
	...
	//设置热钱包参数接口
	function setHotWallteInfoService(reqParam){
		var header = getLocalToken();
		httpService.http('private','setHotWallteInfo','post',{},reqParam,header)
		.then(function success(resp){ 
		    if(httpResponse(resp).success==1){
				createDialog('success','设置热钱包参数成功',1500);
			}else{ 
				createDialog('fail','设置热钱包参数失败',1500);
			}
		},function error(resp){  
	...
	//设置热钱包参数
	$scope.setHotWallteInfo = function(){
		$scope.result.addressobj.asset=$scope.resetParam.asset;
		setHotWallteInfoService($scope.result.addressobj);
	};
